...
byte* readText(const char* fileName, unsigned int *fsize) {
	FILE *f = fopen(fileName, "rb");
	fseek(f, 0, SEEK_END);
	(*fsize) = ftell(f);
	fseek(f, 0, SEEK_SET);

	byte *text = (byte *)malloc((*fsize) + 1);
...
void writeText(string fileName, vector<byte> decodedText) {
	ofstream decompressedFile (fileName);

	decompressedFile.write((const char*)&decodedText[0], decodedText.size());
	decompressedFile.close();

	return ;
}

void readImage(const char *fileName, byte **pixels, int32 *width, int32 *height, int32 *bytesPerPixel, int32 *totalSize)
{
        ...
        *bytesPerPixel = ((int32)bitsPerPixel) / 8;
        ...
        int32 paddedRowSize = (int)(4 * ceil((float)(*width) / 4.0f))*(*bytesPerPixel);
...
void writeImage(const char *fileName, byte *pixels, int32 width, int32 height,int32 bytesPerPixel)
{
        ...
        for ( i = 0; i < height; i++)
        {
                ...
                int32 pixelOffset = ((height - i) - 1)*unpaddedRowSize;
